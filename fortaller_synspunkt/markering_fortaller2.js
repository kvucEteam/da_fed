...
var textdata_opg3_Array = [
    ...
    ["og følte, at de havde arbejdet hårdt. De besluttede sig for at gå op mod Nørreport.", "ydre"],
    ["Drengens søster var på vej ned ad Købmagergade, hvor hun krydsede de to veninder, der styrede mod Nørreport. Bitten tog sit røde uldtørklæde på.", "ydre"],
...
var fortaeller_Array = Array("Alvidende", "Personal");
...
function init() {
    ...
    $(".sentence").click(function(e) {
        ...
        if (!$(this).hasClass("false") && !$(this).hasClass("correct")) {
            ...
            alert("Du har klikket på en dialogisk dimmer!");
        }
    });
    $(".sentence").css("cursor", "pointer");
    $(".correct_words").html(score + " ud af " + correct_words + " rigtige sætninger. Fejl: " + fejl + ")");
    //alert("init end")
}
...
function tjek_svar(input) {
    ...
    if (input != "btn") {
        for (var t = 0; t < $(".sentence").length; t++) {
            //console.log("t: " + t);
            //  if ($(".sentence").eq(t).className.split(' ')[1]){
            if (!$(".sentence").eq(t).hasClass("correct")) {
                if ($(".sentence").eq(t).hasClass("ydre") && textdata_opgaver_Array[runde][t][1] == "ydre") {
                    score++;
                    //alert ("score");
                    $(".sentence").eq(t).addClass("correct");
                    $(".correct_words").html(score + " ud af " + correct_words + " rigtige sætninger. Fejl: " + fejl + ")");
                    return ("ud");
                } else if ($(".sentence").eq(t).hasClass("indre") && textdata_opgaver_Array[runde][t][1] == "indre") {
                    score++;
                    $(".sentence").eq(t).addClass("correct");
                    $(".correct_words").html(score + " ud af " + correct_words + " rigtige sætninger. Fejl: " + fejl + ")");
                    return ("ud");
        ...
        if (runde < textdata_opg_Array.length - 1) {
            runde++;
            alertBox("Alt korrekt", "Du har fundet alle de ord der identificerer fortælleren samt fundet den rigtige fortæller.")
            init();
        } else {
            runde = 0;
        ...
        alertBox("Forkert fortæller", "Du skal finde alle de rigtige ord og vælge hvilken person fortælleren er før du kan gå videre.");
    } else if (input == "word" && score >= antal_korrekte && selected_pers != textdata_opg_Array[runde][0]) {
        alertBox("Alle ord korrekt!", "Du skal finde den rigtige fortæller før du kan gå videre.");
    }*/
        //alert ("score: " + score);
        if (fejl_bool == true) {
            fejl++;
